...
class Metrics:
    ...
    def add(self, query_idx, query_key, ranking, gold_positives):
        ...
        positives = [i for i, (_, pid, _) in enumerate(
            ranking) if pid in gold_positives]
        ...
        for depth in self.mrr_sums:
            ...
            self.mrr_sums[depth] += (1.0 /
                                     (first_positive +
                                      1.0)) if first_positive < depth else 0.0
        ...
        for depth in self.recall_sums:
            num_positives_up_to_depth = len(
                [pos for pos in positives if pos < depth])
            self.recall_sums[depth] += num_positives_up_to_depth / \
                len(gold_positives)

    def print_metrics(self, query_idx):
        for depth in sorted(self.mrr_sums):
            print("MRR@" + str(depth), "=",
                  self.mrr_sums[depth] / (query_idx + 1.0))

        for depth in sorted(self.success_sums):
            print("Success@" + str(depth), "=",
                  self.success_sums[depth] / (query_idx + 1.0))

        for depth in sorted(self.recall_sums):
            print("Recall@" + str(depth), "=",
                  self.recall_sums[depth] / (query_idx + 1.0))
    ...
    def log(self, query_idx):
        ...
        Run.log_metric(
            "ranking/num_queries_added",
            self.num_queries_added,
            query_idx)
...
def evaluate_recall(qrels, queries, topK_pids):
    ...
    recall_at_k = [len(set.intersection(set(qrels[qid]), set(
        topK_pids[qid]))) / max(1.0, len(qrels[qid])) for qid in qrels]
    recall_at_k = sum(recall_at_k) / len(qrels)
    recall_at_k = round(recall_at_k, 3)
    print("Recall @ maximum depth =", recall_at_k)


# TODO: If implicit qrels are used (for re-ranking), warn if a recall
# metric is requested + add an asterisk to output.
