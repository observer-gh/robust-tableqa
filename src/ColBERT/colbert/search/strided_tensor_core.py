...
class StridedTensorCore:
    ...
    def __init__(self, packed_tensor, lengths, dim=None, use_gpu=True):
        ...
        self.lengths = lengths.long() if torch.is_tensor(
            lengths) else torch.LongTensor(lengths)

        self.strides = _select_strides(
            self.lengths, [.5, .75, .9, .95]) + [self.lengths.max().item()]
        ...
        if self.offsets[-2] + self.max_stride > self.tensor.size(0):
            ...
            # print("#> WARNING: Consider doing this padding in advance to save
            # memory!")
        ...
        self.views = {
            stride: _create_view(
                self.tensor,
                stride,
                self.inner_dims) for stride in self.strides}
...
def _get_quantiles(lengths, quantiles):
    return torch.quantile(lengths.float(), torch.tensor(
        quantiles, device=lengths.device)).int().tolist()
