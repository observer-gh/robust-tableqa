...
class Examples:
    ...
    def tolist(self, rank=None, nranks=None):
        ...
        if rank or nranks:
            ...
            return [self.data[idx] for idx in range(0, len(self.data), nranks)]

        return list(self.data)

    def save(self, new_path):
        assert 'json' in new_path.strip(
            '/').split('/')[-1].split('.'), "TODO: Support .json[l] too."
        ...
        with Run().open(new_path, 'w') as f:
            ...
            output_path = f.name
            print_message(
                f"#> Saved examples with {len(self.data)} lines to {f.name}")
