...
class QueryTokenizer():
    def __init__(self, config: ColBERTConfig):
        ...
        self.background_maxlen = 512 - self.query_maxlen + \
            1  # FIXME: Make this configurable
    ...
    def tokenize(self, batch_text, add_special_tokens=False):
        ...
        prefix, suffix = [
            self.cls_token, self.Q_marker_token], [
            self.sep_token]
    ...
    def tensorize(self, batch_text, bsize=None, context=None):
        ...
        if context is not None:
            assert len(context) == len(
                batch_text), (len(context), len(batch_text))
            ...
            # Skip the first [SEP]
            ids_2, mask_2 = obj_2['input_ids'][:,
                                               1:], obj_2['attention_mask'][:, 1:]
