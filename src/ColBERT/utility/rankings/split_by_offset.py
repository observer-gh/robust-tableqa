...
def main(args):
    output_paths = ['{}.{}'.format(args.ranking, split)
                    for split in args.names]
    ...
    with open(args.ranking) as f:
        for line in f:
            ...
            split_output_path.write(
                '\t'.join([str(x) for x in [qid, pid, rank, *other]]) + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int)  # larger than any individual query set
