...
class ColBERTExecutor(BaseExecutor):
    def __init__(self, config, data_loader):
        ...
        if '$' in self.config.model_config.EncoderModelVersion:
            self.config.model_config.EncoderModelVersion = os.path.join(
                self.config.DATA_FOLDER,
                self.config.model_config.EncoderModelVersion.replace(
                    '$',
                    ''))
        ...
        logger.info(
            f'tokenizer lengths = {len(self.tokenizer.tok)} and {len(self.decoder_tokenizer.tok)}')
    ...
    def training_step(self, sample_batched, batch_idx):
        ...
        if config.use_ib_negatives:
            ...
            labels = torch.zeros(
                sample_batched['input_ids'].shape[0] *
                dist.get_world_size(),
                dtype=torch.long,
                device=self.device)
    ...
    def evaluate_outputs(
        ...
        if "exhaustive_search_in_testing" not in self.config.model_config.modules:
            with Run().context(RunConfig(nranks=1, rank=self.global_rank, root=self.config.saved_model_path, experiment="temp_index")):
                ...
                custom_quries = {
                    question_id: question for question_id,
                    question in zip(
                        question_ids,
                        questions)}
                queries = Queries(data=custom_quries)
                ranking = searcher._search_all_Q(
                    queries, query_embeddings, k=max(Ks))
            ...
            for i_batch_id in tqdm(range(n_item_batchs)):
                ...
                retrieved_item_embeddings = np.stack(
                    item_embeddings[i_start:i_end])
                ...
                retrieved_item_embedding_mask = torch.from_numpy(
                    retrieved_item_embedding_mask).to(self.device)
                current_i_size = len(retrieved_item_embeddings)

                # self.model.colbert_config.nway = current_i_size
                Q_duplicated = query_embeddings.repeat_interleave(
                    current_i_size, dim=0).contiguous().to(self.device)
            ...
            for query_index in range(len(query_embeddings)):
                ...
                ranking_list = [
                    (table_indices[i].numpy(),
                     i,
                     table_scores[i].numpy()) for i in range(
                        max(Ks))]
        ...
        for question_id, pos_item_id, ranking_list in zip(
                question_ids, pos_item_ids, ranking_dict.values()):
            ...
            if len(ranking_list) < max_K:
                ...
                # In this case, we simply replicate the last element to avoid
                # crash
                retrieved_table_indices += [retrieved_table_indices[-1]
                                            ] * (max_K - len(ranking_list))
                score += [score[-1]] * (max_K - len(ranking_list))

            retrieved_tables_sorted = table_dataset.select(
                retrieved_table_indices)['table_id']
            ...
            for retrieved_table_id, retrieved_table_score in zip(
                ...
                table_entry += [
                    f"{retrieved_table_id}, {retrieved_table_id in pos_item_id}, {retrieved_table_score}"]
    ...
    def prepare_item_embeddings(self, current_data_loader, mode):
        ...
        if self.global_rank == 0 and not exhaustive_search_in_testing:
            logger.info(
                f"Global rank {self.global_rank} starts indexing job...")
        ...
        # Use barrrier to sync all ranks. Only when Rank 0 finishes indexing,
        # other ranks will move on
        torch.distributed.barrier()
        torch.cuda.empty_cache()

        self.model = self.model.to(self.device)

        if (self.trainer.state.stage == 'test' and self.global_rank == 0) or (
                exhaustive_search_in_testing):
            # Add embeddings
            # For easier implementation, we just rerun the item embedding
            # generation here
            i_count = 0
            item_embeddings = []
            item_embedding_mask = []
            for i_batch_id in tqdm(range(n_item_batchs)):
                ...
                item_input_ids, item_attention_mask = self.data_loader.decoder_tokenizer.tensorize(
                ...
                # batch_size x hidden_states
                item_emb, item_emb_mask = self.model.doc(
                    *test_batch, keep_dims='return_mask')
        ...
        # Sync all processes. If rank 0 starts saving item embeddings in
        # testing, other processes will wait for it.
        torch.distributed.barrier()
        ...
        if self.trainer.state.stage == 'test' and self.global_rank == 0 and not exhaustive_search_in_testing:
            ...
            index_path_to = os.path.join(
                save_path, "table_dataset_colbert_index")
            logger.info(
                f'saving index files from {index_path_from} into {index_path_to}')
            import shutil
