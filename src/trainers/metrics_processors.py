...
class MetricsProcessor():
    ...
    def compute_tapas_denotation_accuracy(self, module, data_dict, log_dict):
        ...
        def evaluate_example_with_operator(
                predict_float: str, ground_str: str):
            ...
            if len(ref_values) != 1:
                # currently, in the case of using an operator, the reference
                # must be of length 1
                return False
        ...
        def get_denotation_accuracy(predictions, references, pred_aggregation):
            ...
            correct_num = 0
            for predict_str, ground_str, agg_str in zip(
                    predictions, references, pred_aggregation):
                if agg_str == 'NONE':
                    # perform denotation as usual
                    is_correct = evaluate_example(
                        predict_str.lower(), ground_str.lower())
                    ...
                    # check if they match!
                    is_correct = evaluate_example_with_operator(
                        predict_str, ground_str.lower())
                    # print(predict_str, ground_str, is_correct)
                if is_correct:
                    correct_num += 1
            return correct_num / len(predictions)

        def get_denotation_accuracy_with_original_table_overflow(
                predictions, references, pred_aggregation, overflows, raw_cells):
            assert len(predictions) == len(references)
            ...
            overflow_num = 0
            non_overflow_num = 0
            for predict_str, ground_str, agg_str, original_table_overflow, raw_cell_values in zip(
                    predictions, references, pred_aggregation, overflows, raw_cells):
                if agg_str == 'NONE':
                    # perform denotation as usual
                    is_correct = evaluate_example(
            ...
            return overflow_correct_num / len(predictions), non_overflow_correct_num / len(predictions), zero_div(
                overflow_correct_num, overflow_num), zero_div(non_overflow_correct_num, non_overflow_num)
        ...
        final_pred = []
        raw_cells = []
        check_original_table_overflow = (
            'original_table_overflow' in data_dict['batch_predictions'][0].keys())
        ...
        for pred_dict in data_dict['batch_predictions']:
            ...
            if check_original_table_overflow:
                original_table_overflows.append(
                    pred_dict.get('original_table_overflow', True))

        # Some simple post-processing
        decoded_preds, decoded_labels = postprocess_text(
            decoded_preds, decoded_labels)

        accuracy = get_denotation_accuracy(
            final_pred, decoded_labels, pred_aggregation)
        ...
        if check_original_table_overflow:
            overflow_accuracy_global, non_overflow_accuracy_global, overflow_accuracy, non_overflow_accuracy = get_denotation_accuracy_with_original_table_overflow(
                decoded_preds, decoded_labels, pred_aggregation, original_table_overflows, raw_cells)
    ...
    def compute_denotation_accuracy(self, module, data_dict, log_dict):
        ...
        decoded_preds = [pred_dict['decoded_output']
                         for pred_dict in data_dict['batch_predictions']]
        decoded_labels = [pred_dict['decoded_label']
                          for pred_dict in data_dict['batch_predictions']]
        ...
        if check_original_table_overflow:
            original_table_overflows = [
                pred_dict.get(
                    'original_table_overflow',
                    True) for pred_dict in data_dict['batch_predictions']]
        ...
        def get_denotation_accuracy_with_original_table_overflow(
                predictions, references, overflows):
            assert len(predictions) == len(references)
            assert len(predictions) == len(overflows)
            overflow_correct_num = 0
            non_overflow_correct_num = 0
            overflow_num = 0
            non_overflow_num = 0
            for predict_str, ground_str, original_table_overflow in zip(
                    predictions, references, overflows):
                is_correct = evaluate_example(
                    predict_str.lower(), ground_str.lower())
                if original_table_overflow:
        ...
        if check_original_table_overflow:
            overflow_accuracy_global, non_overflow_accuracy_global, overflow_accuracy, non_overflow_accuracy = get_denotation_accuracy_with_original_table_overflow(
                decoded_preds, decoded_labels, original_table_overflows)
    ...
    def compute_accuracy_with_alternative_answers(
            self, module, data_dict, log_dict):
        ...
        decoded_labels = [pred_dict['decoded_label']
                          for pred_dict in data_dict['batch_predictions']]
        alternative_labels = [pred_dict['alternative_answers']
                              for pred_dict in data_dict['batch_predictions']]
        ...
        def get_denotation_accuracy(
                predictions,
                references,
                alternative_answers):
            assert len(predictions) == len(references)
            correct_num = 0
            for predict_str, ground_str, alternative_strs in zip(
                    predictions, references, alternative_answers):
                is_correct = False
        ...
        accuracy = get_denotation_accuracy(
            decoded_preds, decoded_labels, alternative_labels)

        log_dict.metrics['denotation_accuracy'] = accuracy

        return log_dict

    def compute_ITR_RAG_retrieval_results(
            self, module, data_dict, log_dict) -> dict:
        """
        ...
        Ks = [K for K in Ks if K <=
              self.config.data_loader.additional.num_knowledge_passages]
        ...
        for retrieval_result in batch_results:
            ...
            for retrieved_sub_table, rating in retrieved_tables:
                if gold_reference.issubset(
                        set(retrieved_sub_table[indice_ref])):
            ...
            # whether there is a non-overflow full recall in the retrieved sub
            # tables
            ...
            # whether an originally overflow table gets non-overflow full
            # recall
            overflow_resolve_success = non_overflow_full_recall and original_table_overflow
            overflow_resolve_fail = (
                not non_overflow_full_recall) and original_table_overflow
            ...
            for index, K in enumerate(Ks):
                ...
                truncated_num_hit_list = num_hit_list[:min(
                    K, len(num_hit_list))]

                # reduce full recall measures whether all gold columns are
                # collected
        ...
        for metrics_name, np_array in result.items():
            if metrics_name not in ['full_recall']:
                ...
                for index, K in enumerate(Ks):
                    log_result[f'{metrics_name}_at_{K}'] = float(
                        np_array[index])
        log_dict.metrics.update(log_result)
        return log_dict

    def compute_ITR_mix_RAG_retrieval_results(
            self, module, data_dict, log_dict) -> dict:
        """
        Evaluate the retrieval performance of Inner Table Retrieval - RAG Mode
        Args:
        ...
        for retrieval_result in batch_results:
            ...
            for retrieved_sub_table, rating in retrieved_tables:
                retrieved_cells = get_cells_from_col_row_index(
                    retrieved_sub_table['sub_row_indice'],
                    retrieved_sub_table['sub_column_indice'])
    ...
    def compute_ITR_retrieval_results(
            self, module, data_dict, log_dict) -> dict:
        """
        ...
        for retrieval_result in batch_results:
            ...
            num_hit_list = []
            for retrieved_sub_table, rating in retrieved_tables:
                num_hit_list.append((retrieved_sub_table['is_gold']))
            for index, K in enumerate(Ks):
                # obtain the list wrt K
                truncated_num_hit_list = num_hit_list[:min(
                    K, len(num_hit_list))]
                ...
                precision_at_K = sum(truncated_num_hit_list) / \
                    len(truncated_num_hit_list)
                # full recall measures whether all gold columns are collected
                full_recall_at_K = (
                    len(retrieval_result[gold_ref]) <= sum(truncated_num_hit_list))
    ...
    def compute_ITR_mix_retrieval_results(
            self, module, data_dict, log_dict) -> dict:
        """
        ...
        for retrieval_result in batch_results:
            ...
            for retrieved_sub_table, rating in retrieved_tables:
                ...
                current_cells = get_cells_from_col_row_index(
            ...
            for index, K in enumerate(Ks):
                ...
                precision_at_K = sum(truncated_num_hit_list) / \
                    len(truncated_num_hit_list)
                # full recall measures whether all gold columns are collected
                full_recall_at_K = gold_cells.issubset(
                    retrieved_cells_all[min(K, len(retrieved_cells_all)) - 1])
                result['recall'][index] += recall_at_K
                result['precision'][index] += precision_at_K
                result['full_recall'][index] += full_recall_at_K
    ...
    def compute_TQA_DPR_scores(self, module, data_dict, log_dict) -> dict:
        ...
        for retrieved_results in batch_results:
            ...
            hit_list = [
                retrieved_table_id in pos_item_ids for retrieved_table_id in retrieved_tables_sorted]
            for index, K in enumerate(Ks):
    ...
    def compute_RAG_retrieval_results(
            self, module, data_dict, log_dict) -> dict:
        """
    ...
    def compute_token_f1(self, module, data_dict, log_dict):
        ...
        def compute_f1(a_gold, a_pred):
            ...
            common = collections.Counter(
                gold_toks) & collections.Counter(pred_toks)
            num_same = sum(common.values())
            if len(gold_toks) == 0 or len(pred_toks) == 0:
                # If either is no-answer, then F1 is 1 if they agree, 0
                # otherwise
