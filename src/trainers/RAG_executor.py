...
class RAGExecutor(BaseExecutor):
    ...
    def configure_optimizers(self):
        ...
        if weight_decay == 0:
            optimization_parameters = [
                {
                    'params': [
                        p for n,
                        p in self.model.named_parameters() if 'generator' in n and p.requires_grad],
                ...
                },
                {
                    'params': [
                        p for n,
                        p in self.model.named_parameters() if 'generator' not in n and p.requires_grad],
            ...
            optimization_parameters = [{"params": [p for n,
                                                   p in self.model.named_parameters() if n in decay_parameters and 'generator' in n and p.requires_grad],
                                        "weight_decay": weight_decay,
                                        'lr': self.config.train.lr,
                                        'initial_lr': self.config.train.lr,
                                        },
                                       {'params': [p for n,
                                                   p in self.model.named_parameters() if n in decay_parameters and 'generator' not in n and p.requires_grad],
                                        'lr': self.config.train.retriever_lr,
                                        'initial_lr': self.config.train.retriever_lr,
                                        },
                                       {"params": [p for n,
                                                   p in self.model.named_parameters() if n not in decay_parameters and 'generator' in n and p.requires_grad],
                                        "weight_decay": 0.0,
                                        'lr': self.config.train.lr,
                                        'initial_lr': self.config.train.lr,
                                        },
                                       {'params': [p for n,
                                                   p in self.model.named_parameters() if n not in decay_parameters and 'generator' not in n and p.requires_grad],
                                        'lr': self.config.train.retriever_lr,
                                        'initial_lr': self.config.train.retriever_lr,
                                        },
                                       ]
    ...
    def _generative_step(self, sample_batched, batch_idx):
        ...
        for index, i in enumerate(labels):
            ...
            cleaned_i = [
                label if label != -
                100 else self.decoder_tokenizer.pad_token_id for label in output_sequence]
            cleaned_i = torch.LongTensor(cleaned_i)
            ...
            predictions.append(
                {
                    ...
                    'alternative_answers': [
                        ", ".join(ans) for ans in sample_batched['alternative_answers'][index]],
            ...
            for doc, doc_prediction in zip(
                    retrieved_docs[index], generation_outputs_for_docs[index]):
    ...
    def evaluate_outputs(self, step_outputs, mode='test'):
        ...
        batch_loss_with_doc_scores = np.concatenate(
            batch_loss_with_doc_scores, axis=0)
