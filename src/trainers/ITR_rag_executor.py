...
class ITRRAGExecutor(BaseExecutor):
    ...
    def _generative_step(self, sample_batched, batch_idx):
        ...
        data_to_return = {
            ...
            'generator_input_sub_tables': generator_input_sub_tables,
    ...
    def evaluate_outputs(self, step_outputs, mode='test'):
        ...
        batch_doc_scores = []
        batch_question_ids = []
        batch_generator_input_sub_tables = []
        batch_sub_tables = []
        gold_column_dict = {}
        gold_row_dict = {}
        ...
        data_used_for_metrics = EasyDict(
            ...
            batch_retrieval_results=batch_retrieval_results,
        )
...
class ITRRAGExecutorForTAPAS(ITRRAGExecutor):
    ...
    def _generative_step(self, sample_batched, batch_idx):
        ...
        doc_scores = generation_outputs.doc_scores
        generator_input_sub_tables = generation_outputs.generator_input_sub_tables
        agg_predictions = generation_outputs.agg_predictions
        raw_cell_values = generation_outputs.raw_cell_values
        ...
        for index, question_id, answer_list, query, pred, predicted_agg, valid, raw_cells in zip(range(len(
            ...
            if isinstance(pred, list):
                ...
                decoded_output = ", ".join(pred)
            else:
                # the prediction is not a list
                # aggregation -> transform to string
                decoded_output = str(pred)
