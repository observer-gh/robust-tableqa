...
class ITRDPRExecutor(BaseExecutor):
    ...
    def evaluate_outputs(
        ...
        for step_output in step_outputs:
            ...
            for question_id, sub_table_list in zip(
                    step_output['question_ids'], step_output['sub_tables']):
                # question_id: (start_index, end_index)
                question_id_to_sub_table_index[question_id] = (
                    len(sub_tables), len(sub_tables) + len(sub_table_list))
        ...
        for i_batch_id in tqdm(range(n_item_batchs)):
            ...
            i_rate_batch = torch.matmul(
                query_embeddings, item_emb.t()).detach().cpu()
        ...
        for question_id, start_end_index in tqdm(
                question_id_to_sub_table_index.items()):
            start, end = start_end_index
            query_index = question_id_to_query_index[question_id]
            # 1 x end-start
            query_sub_table_ratings = rate_batch[query_index, start:end]
            query_sub_tables = sub_tables[start:end]  # length: end-start
            # zip and sort sub_tables wrt scores
            zipped_query_sub_tables = [
                (query_sub_table,
                 query_sub_table_rating) for query_sub_table,
                query_sub_table_rating in zip(
                    query_sub_tables,
                    query_sub_table_ratings)]
        ...
        if self.trainer.state.stage == 'test':
            save_path = os.path.join(
                self.config.results_path, 'step_{}'.format(
                    self.global_step), dataset_name.replace(
                    '/', '.'))
            create_dirs([save_path])
            save_path = os.path.join(
                save_path, f'static_index_{self.global_rank}.pkl')
            to_save_data = dict(
