...
class TAPASExecutor(BaseExecutor):
    ...
    def training_step(self, sample_batched, batch_idx):
        ...
        # if label smoother is defined, pass the logits and labels to the
        # smoother
        if self.label_smoother is not None:
            if unwrap_model(self.model)._get_name(
            ) in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
                batch_loss = self.label_smoother(
                    forward_results, train_batch.labels, shift_labels=True)
            else:
                batch_loss = self.label_smoother(
                    forward_results, train_batch.labels)
        else:
    ...
    def _prediction_step(self, sample_batched, batch_idx):
        ...
        for question_id, gold_answer, query, answer, predicted_agg, valid in zip(
                question_ids, gold_answers, input_text_sequences, answers, aggregation_predictions_string, valids):
            # Transform aggregation to a final prediction
            if predicted_agg == "SUM":
                all_cell_values = answer.split(', ')
                all_cell_values = [cell_value.strip()
                                   for cell_value in all_cell_values]
                try:
                    all_cell_values = [float(cell_value)
                                       for cell_value in all_cell_values]
