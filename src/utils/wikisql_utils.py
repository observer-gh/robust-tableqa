...
def _get_answer_coordinates(table, sql_query):
    ...
    if not indices:
        return [], aggregation_op, conditions

    if len(indices) == 1:
        return indices, aggregation_op, conditions
    ...
    if aggregation_op_index in (1, 2):
        ...
        return ret, _Aggregation.NONE, conditions
...
def retrieve_wikisql_query_answer_tapas(table, example) -> List:
    ...
    return answer_text, answer_coordinates, aggregation_op, conditions
