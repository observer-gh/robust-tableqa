...
class ITRRagModel(pl.LightningModule):
    ...
    def __init__(self, config: EasyDict, data_loader) -> None:
        ...
        if "$" in self.config.model_config.QueryEncoderModelVersion:
            # it is a local path, we need to load parameters manually to avoid
            # a bug of Huggingface
        ...
        GeneratorModelClass = globals()[
            self.config.model_config.GeneratorModelClass]
        GeneratorConfigClass = globals()[
            self.config.model_config.GeneratorConfigClass]
        ...
        if 'tapex' in self.config.model_config.GeneratorModelVersion:
            ...
            # we should disable it here to avoid problematic generation (used
            # for TAPEX model only)
        ...
        self.generator = GeneratorModelClass.from_pretrained(
            self.config.model_config.GeneratorModelVersion, config=generator_model_config)
        ...
        if self.config.model_config.get(
            "LoadGeneratorModelPath",
                None) is not None:
            ...
            pretrained_dict = {
                "generator." + k[6:]: v for k, v in pretrained_state_dict.items() if k.startswith("model.")}

            # print(pretrained_dict.keys())
            pretrained_dict = {
                k: v for k,
                v in pretrained_dict.items() if k in model_state_dict.keys()}
            print(
                'Loading pretrained weights', [
                    k for k in pretrained_dict.keys()])
            # 2. overwrite entries in the existing state dict
            model_state_dict.update(pretrained_dict)
            # 3. load the new state dict
            self.load_state_dict(model_state_dict)
    ...
    def init_retrieval(self):
        ...
        """
        self.sub_table_id_to_embeddings = defaultdict(
            dict)  # question_id -> sub_table_id -> embedding
        # question_id -> sub_table_id -> content
        self.sub_table_id_to_content = defaultdict(dict)
        ...
        for split, split_index_file_path in index_files.index_paths.items():
            split_index_file_path = os.path.join(
                self.config.EXPERIMENT_FOLDER, split_index_file_path)
            all_shard_files = glob.glob(
                f"{split_index_file_path}/static_index_*.pkl")
            for file_path in all_shard_files:
                ...
                for question_id in question_ids:
                    ...
                    self.question_id_to_query_embedding[question_id] = query_embeddings[
                        question_id_to_query_index[question_id]]

                    if self.question_id_to_sub_table_ids.get(
                            question_id, None) is not None:
                        logger.warning(
                            f'{file_path} found duplicated question_id={question_id} sub_tables {len(current_sub_tables)}')

                    for sub_table, sub_table_emb in zip(
                            current_sub_tables, current_sub_table_embeddings):
                        sub_table_id = sub_table['id']
                        self.question_id_to_sub_table_ids.setdefault(
                            question_id, []).append(sub_table['id'])
                        self.sub_table_id_to_content[question_id][sub_table['id']] = sub_table
                        self.sub_table_id_to_embeddings[question_id][sub_table['id']
                                                                     ] = sub_table_emb
                # break # debug, load only one shard
                logger.info(
                    f"number of questions {len(self.question_id_to_sub_table_ids)}")
                logger.info(
                    f"number of sub tables {len(self.sub_table_id_to_content)}")

        for question_id, list_sub_tables in self.question_id_to_sub_table_ids.items():
            self.question_id_to_sub_table_ids[question_id] = list(
                set(list_sub_tables))
    ...
    def _get_top_sub_tables_from_n_gram(self,
        ...
        def return_n_gram(string, n_gram):
            ...
            return [" ".join(words[index:index + n_gram])
                    for index in range(len(words) - n_gram + 1)]
        n_gram_list = self.config.model_config.get('n_gram_list', [1, 2, 3])

        for question_id, question, table, query_embedding in zip(
                question_ids, input_text_sequences, tables, query_hidden_states):
            sub_table_ids = self.question_id_to_sub_table_ids[question_id]
            sub_tables = [self.sub_table_id_to_content[question_id]
                          [sub_table_id] for sub_table_id in sub_table_ids]
            sub_table_embeddings = [self.sub_table_id_to_embeddings[question_id][
                sub_table_id] for sub_table_id in sub_table_ids]  # list of emb_size

            # num_subtables x emb_size
            sub_table_embeddings = torch.stack(sub_table_embeddings)

            # move the embeddings to gpu
            sub_table_embeddings = sub_table_embeddings.to(
            ...
            for sub_table in sub_tables:
                ...
                for n_gram in n_gram_list:
                    ...
                    column_content_n_grams = return_n_gram(
                        " ".join(column_content), n_gram)
                    overlap = set(column_content_n_grams).intersection(
                        set(question_n_grams))
                    sub_table_score += len(overlap)
                sub_table_ratings.append(sub_table_score)

            sub_table_ratings = torch.FloatTensor(
                sub_table_ratings).to(query_embedding.device)

            # # We return all ranked sub tables
            # Sort scores
            sorted_ratings, indices = torch.sort(
                sub_table_ratings, descending=True)
            ranked_scores = sorted_ratings  # [:k]
            ranked_indices = indices  # [:k]

            ranked_sub_tables = [
                self.sub_table_id_to_content[question_id][
                    sub_table_ids[index]] for index in ranked_indices]
    ...
    def _get_top_sub_tables_from_index(self,
        ...
        for question_id, query_embedding in zip(
                question_ids, query_hidden_states):
            sub_table_ids = self.question_id_to_sub_table_ids[question_id]
            ...
            sub_table_ratings = torch.matmul(
                query_embedding, sub_table_embeddings.t())
    ...
    def main_retrieve(self,
        ...
        question_hidden_states = torch.stack(
            [self.question_id_to_query_embedding[question_id] for question_id in question_ids]).to(input_ids.device)
    ...
    def prepare_inputs_for_generator(self,
        ...
        original_sub_table_order = (
            'original_sub_table_order' in self.config.model_config.modules)

        def reduce_table(original_table, ranked_sub_tables):
            table_pd = pd.DataFrame.from_records(
                original_table['rows'], columns=original_table['header'])
            column_index_to_content = {
                header_index: header_name for header_index,
                header_name in enumerate(
            ...
            for i in ranked_sub_tables:
                ...
                if original_sub_table_order:
                    processed_sub_table['sub_column_indice'] = sorted(
                        processed_sub_table['sub_column_indice'])

                selected_table_pd = table_pd.iloc[:,
                                                  processed_sub_table['sub_column_indice']]
                processed_sub_table['header'] = [column_index_to_content[index]
                                                 for index in processed_sub_table['sub_column_indice']]
                processed_sub_table['rows'] = selected_table_pd.values.tolist()
                processed_sub_table['types'] = [original_table['types'][index]
                                                for index in processed_sub_table['sub_column_indice']]
        ...
        for index, question_id, input_text_sequence, gold_column_list, ranked_sub_tables, table in zip(
                range(batch_size), question_ids, input_text_sequences, gold_columns, retrieved_sub_tables, tables):
            ...
            input_text_and_sub_tables = [
                (input_text_sequence, sub_table) for sub_table in processed_sub_tables]

            if not is_training:
                encoding = self.generator_tokenizer(
                    query=[
                        text_table_tuple[0] for text_table_tuple in input_text_and_sub_tables],
                    table=[
                        pd.DataFrame.from_records(
                            text_table_tuple[1]["rows"],
                            columns=text_table_tuple[1]["header"]) for text_table_tuple in input_text_and_sub_tables],
                    padding='longest',
                    truncate=False,
                    return_tensors="pt",
                )
                ...
                if overflow_only and not original_table_overflow:
                    ...
                    token_lengths = [(i, token_lengths[i])
                ...
                # in training, just make sure the last element has all gold
                # columns
                token_lengths = [(i, 0)
                                 for i in range(len(processed_sub_tables))]

                # earliest_element_that_covers_all_gold = len(token_lengths)-1
                # use earliest_element_that_covers_all_gold = -1 for
                # unsupervised case
                ...
                for index, subtable in enumerate(processed_sub_tables):
                    cover_all_gold = set(gold_column_list).issubset(
                        set(subtable['sub_column_indice']))
                    if cover_all_gold:
                ...
                token_lengths = [
                    token_lengths[earliest_element_that_covers_all_gold]] * n_docs

            if n_docs > len(token_lengths):
                # repeat the last item
                token_lengths_reduced = token_lengths + \
                    [token_lengths[-1]] * (n_docs - len(processed_sub_tables))
            else:
                # take the first n_docs sub tables
                token_lengths_reduced = token_lengths[:n_docs]

            extended_input_text_sequences += [input_text_and_sub_tables[i]
                                              for i, _ in token_lengths_reduced]
            input_sub_tables.append(
                [input_text_and_sub_tables[i][1] for i, _ in token_lengths_reduced])

        # ensure that when not all gold col are retrieved, the model tries to
        # learn sth.
        mask[:, -1] = 1
        ...
        encoding = self.generator_tokenizer(
            query=[
                text_table_tuple[0] for text_table_tuple in extended_input_text_sequences],
            table=[
                pd.DataFrame.from_records(
                    text_table_tuple[1]["rows"],
                    columns=text_table_tuple[1]["header"]) for text_table_tuple in extended_input_text_sequences],
            padding='longest',
            max_length=self.config.data_loader.additional.max_decoder_source_length,
            truncation=True,
            return_tensors="pt")
        generator_input_ids, generator_attention_mask, generator_token_type_ids = encoding.input_ids, encoding.attention_mask, encoding.get(
            "token_type_ids", None)
        ...
        if targets is not None:
            generator_decoder_input_ids = shift_tokens_right(
                ...
                self.generator.config.decoder_start_token_id)
        else:
    ...
    def generate(self, input_ids: torch.Tensor,
        ...
        loss_with_doc_scores = - \
            generation_seq_scores.reshape(batch_size, n_docs)
        ...
        for b in range(batch_size):
            ...
            answer_proposals = generation_outputs_decoded[b * n_docs:(
                b + 1) * n_docs]
        ...
        return EasyDict(
            ...
            doc_scores=[
                doc_score.cpu().detach().numpy() for doc_score in doc_scores],
    ...
    def non_autoregressive_predict(self, input_ids: torch.Tensor,
        ...
        for index, coordinates in enumerate(predicted_answer_coordinates):
            ...
            table = pd.DataFrame.from_records(
                table['rows'], columns=table['header'])
            all_cell_values = []
            ...
            if predicted_agg == "NONE":
                ...
                if len(all_cell_values) == 0 and (
                        predicted_agg == "SUM" or predicted_agg == "AVERAGE"):
                    ...
                    if predicted_agg == 'COUNT':
                        ...
                        try:
                            all_cell_values = [
                                convert_to_float(value) for value in all_cell_values]
                            if predicted_agg == "SUM":
                                final_pred = sum(all_cell_values)
                            elif predicted_agg == 'AVERAGE':
                                final_pred = sum(
                                    all_cell_values) / len(all_cell_values)
        ...
        try:
            for b in range(batch_size):
                ...
                top_cand_inds = [-1]
                outputs.append(
                    generation_outputs_decoded[b * n_docs:(b + 1) * n_docs][top_cand_inds[0]])
                agg_predictions.append(
                    aggregation_predictions_string[b * n_docs:(b + 1) * n_docs][top_cand_inds[0]])
                answer_proposals = generation_outputs_decoded[b * n_docs:(
                    b + 1) * n_docs]
                generation_outputs_for_docs.append(answer_proposals)
                # pprint(answer_proposals)
            ...
            print(self.generator_tokenizer.batch_decode(
                generator_inputs.generator_input_ids))
    ...
    def get_loss(
        ...
        use_bos = bos_token_id is not None and target[:, 0].eq(
            bos_token_id).all()
        ...
        if loss_mask is not None:
            ...
            ll.masked_fill_(loss_mask.unsqueeze(-1).repeat(1,
                            1, ll.shape[-1]).bool().eq(0), 0.0)
    ...
    def DistanceCorrelation(tensor_1, tensor_2):
        ...
        a, b = torch.sqrt(torch.max(tensor_1_square -
                                    a_ +
                                    tensor_1_square.t(), zeros) +
                          1e-8), torch.sqrt(torch.max(tensor_2_square -
                                                      b_ +
                                                      tensor_2_square.t(), zeros) +
                                            1e-8)  # [channel, channel]
        """cul distance correlation"""
        A = a - a.mean(dim=0, keepdim=True) - \
            a.mean(dim=1, keepdim=True) + a.mean()
        B = b - b.mean(dim=0, keepdim=True) - \
            b.mean(dim=1, keepdim=True) + b.mean()
        dcov_AB = torch.sqrt(
            torch.max(
                (A * B).sum() / channel ** 2,
                zero) + 1e-8)
    ...
    def _get_top_sub_tables_from_n_gram(self,
        ...
        for question_id, question, table, query_embedding in zip(
            ...
            for sub_table in sub_tables:
                ...
                for n_gram in n_gram_list:
                    question_n_grams = return_n_gram(question, n_gram)
                    row_content_n_grams = return_n_gram(
                        " ".join(row_content), n_gram)
                    overlap = set(row_content_n_grams).intersection(
                        set(question_n_grams))
                    sub_table_score += len(overlap)
                sub_table_ratings.append(sub_table_score)
    ...
    def prepare_inputs_for_generator(self,
        ...
        def reduce_table(original_table, ranked_sub_tables):
            ...
            for i in ranked_sub_tables:
                ...
                if original_sub_table_order:
                    processed_sub_table['sub_row_indice'] = sorted(
                        processed_sub_table['sub_row_indice'])
        ...
        mask = torch.ones(batch_size, n_docs).to(self.device)
        input_sub_tables = []
        for index, question_id, input_text_sequence, gold_column_list, gold_row_list, ranked_sub_tables, table in zip(
                range(batch_size), question_ids, input_text_sequences, gold_columns, gold_rows, retrieved_sub_tables, tables):
            ...
            if not is_training:
                ...
                if return_encoding(
                        processed_sub_tables[-1]).input_ids.shape[1] > max_decoder_source_length:
                ...
                # pprint([subtable['sub_column_indice'] for subtable in processed_sub_tables])
                for index, subtable in enumerate(processed_sub_tables):
                    cover_all_gold = set(gold_row_list).issubset(
                        set(subtable['sub_row_indice']))
                    if cover_all_gold:
                        earliest_element_that_covers_all_gold = index
                        break
