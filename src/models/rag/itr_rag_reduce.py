        ...
        for index, question_id, input_text_sequence, gold_column_list, ranked_sub_tables, table in zip(
            ...
            if not is_training:
                ...
                if overflow_only and not original_table_overflow:
                    ...
                    token_lengths_reduced = [
                        (i,
                         length) for i,
                        length in token_lengths if length <= self.config.data_loader.additional.max_decoder_source_length]

                    # print('after reduction:', token_lengths_reduced)
                    if len(token_lengths_reduced) == 0:
                        # Case 2: if it does not even fit one table
                        logger.warning(
                            f"question {question_id} table {table['id']} can not fit!")
                        ...
                        # Case 3
                        token_lengths_reduced = token_lengths[:min(
                            n_docs, len(token_lengths))]
                    else:
                        # Case 1
                ...
                if self.config.model_config.get('supervised', True):
            ...
            if n_docs > len(token_lengths_reduced):
                ...
                token_lengths_reduced = [token_lengths_reduced[0]] * \
                    (n_docs - len(token_lengths_reduced)) + token_lengths_reduced
            else:
                token_lengths_reduced = token_lengths_reduced[-n_docs:]
...
class ITRRagReduceRowWiseModel(ITRRagModel):
    ...
    def prepare_inputs_for_generator(self,
        ...
        for index, question_id, input_text_sequence, gold_column_list, gold_row_list, ranked_sub_tables, table in zip(
            ...
            processed_sub_tables = reduce_table(table, ranked_sub_tables)
            # we do a binary search to find the index of which the concatenated
            # sub table goes overflow
...
class ITRRagReduceMixModel(ITRRagModel):
    ...
    def prepare_inputs_for_generator(self,
        ...
        shuffle_sub_table_order_in_training = (
            'shuffle_sub_table_order_in_training' in self.config.model_config.modules)
        ...
        def reduce_table(original_table, ranked_sub_tables):
            ...
            for i in ranked_sub_tables:
                ...
                if len(current_column_indices) < self.config.model_config.min_columns or len(
                        current_row_indices) == 0:
                ...
                processed_sub_table['rows'] = selected_table_pd.values.tolist()
                processed_sub_table['types'] = [t for col_index, t in enumerate(
                    original_table['types']) if col_index in current_column_indices]
                processed_sub_tables.append(processed_sub_table)
            return processed_sub_tables
        ...
        for index, question_id, input_text_sequence, gold_column_list, gold_row_list, ranked_sub_tables, table in zip(
            ...
            # processed_sub_tables = reduce_table(table, ranked_sub_tables[:min(max_num_subtables, len(ranked_sub_tables))])
            if random_reduce:
                processed_sub_tables = random_reduce_table(
                    table, ranked_sub_tables)
            else:
                processed_sub_tables = reduce_table(table, ranked_sub_tables)
            # we do a binary search to find the index of which the concatenated
            # sub table goes overflow
            ...
            def binary_search(start, end, verbose=False):
                if start >= end:
                    ...
                    if encoding.input_ids.shape[1] > max_decoder_source_length:
                        if start == 0:
                            logger.error(
                                f"question {question_id} exceeds limit.")
                            return -1
                        else:
                ...
                if verbose:
                    print(
                        f'strt {start} end {end} mid {mid} length {encoding.input_ids.shape[1]}')
                if encoding.input_ids.shape[1] > max_decoder_source_length:
                    # mid term overflow
            ...
            if not is_training:
                ...
                if (overflow_only and not original_table_overflow) or suppress_ITR:
                    ...
                    if random_reduce:
                        ...
                        token_lengths_reduced = [
                            token_lengths[i] for i in range(n_docs)]
                ...
                if self.config.model_config.get('supervised', True):
                    # pprint([subtable['sub_column_indice'] for subtable in processed_sub_tables])
                    for index, subtable in enumerate(processed_sub_tables):
                        cover_all_gold = set(gold_row_list).issubset(set(subtable['sub_row_indice'])) and set(
                            gold_column_list).issubset(set(subtable['sub_column_indice']))
                        if cover_all_gold:
                            earliest_element_that_covers_all_gold = index
                            break
