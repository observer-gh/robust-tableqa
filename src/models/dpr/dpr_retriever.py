...
class RetrieverDPR(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        if self.SEP_ENCODER:
            ...
            self.item_encoder = ItemEncoderModelClass.from_pretrained(
                self.config.model_config.ItemEncoderModelVersion, config=item_model_config)
        else:
        ...
        **kwargs
        ...
        if 'negative_samples_across_gpus' in self.config.model_config.modules:
            ...
            for rank_index, remote_q_embeddings in enumerate(
                    global_query_embeddings_placeholder):
                # We append the embeddings from other GPUs if this embedding
    ...
    def generate_query_embeddings(
        ...
        if self.query_pooler is not None:
            query_last_hidden_states = self.query_pooler(
                query_last_hidden_states)
        query_embeddings = query_last_hidden_states
    ...
    def create_bpr_loss(self, query, pos_items, neg_items):
        ...
        extend_query = query.repeat(
            1, num_neg_samples).reshape(-1, hidden_size)
        ...
        if num_neg_samples > 1:
            ...
            pos_scores = pos_scores.repeat(
                num_neg_samples, 1).permute(1, 0).reshape(-1)
