...
from transformers.utils import ExplicitEnum, PaddingStrategy, TensorType, add_end_docstrings, is_pandas_available, logging
...
class CustomTapasTokenizer(PreTrainedTokenizer):
    r"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.do_extreme_cell_truncation = True
    ...
    def save_vocabulary(
        ...
        if os.path.isdir(save_directory):
            ...
            vocab_file = (
                filename_prefix + "-" if filename_prefix else "") + save_directory
    ...
    def create_attention_mask_from_sequences(
            self,
            query_ids: List[int],
            table_values: List[TableValue]) -> List[int]:
        """
    ...
    def __call__(self,
                 table: Union[List[pd.DataFrame],
                              "pd.DataFrame"],
                 query: Optional[Union[TextInput,
                                       PreTokenizedInput,
                                       EncodedInput,
                                       List[TextInput],
                 ...
                 answer_coordinates: Optional[Union[List[Tuple],
                                                    List[List[Tuple]]]] = None,
                 answer_text: Optional[Union[List[TextInput],
                                             List[List[TextInput]]]] = None,
        ...
        if is_batched:
            # Use batch_encode_plus by multiple times
            assert len(table) == len(
                queries) and isinstance(table, (list, tuple))
            batch_size = len(table)
            batched_encoding = EasyDict({})
            ...
            for index in range(batch_size):
                ...
                encoding = self.encode_plus(
                    table=table[index],
                    query=queries[index],
                    answer_coordinates=answer_coordinates[index] if answer_coordinates is not None else None,
                    answer_text=answer_text[index] if answer_text is not None else None,
                    add_special_tokens=add_special_tokens,
                    padding='max_length' if padding == "longest" else padding,
                ...
                for key, value in encoding.items():
                    # print("get:", key, value.shape)
                    if value.shape[1] > max_length:
                        value = value[:, :max_length]
                        # print("now:", key, value.shape)
                    batched_encoding.setdefault(key, []).append(value)

            for key, value in batched_encoding.items():
                batched_encoding[key] = torch.concat(value)
            if padding == "longest":
                max_ts = (
                    batched_encoding['attention_mask'] == 1).nonzero().numpy()[
                    :, 1].max()
                # print('max_ts', max_ts)
                for key, value in batched_encoding.items():
                    value = value[:, :max_ts + 1]
                    # print('truncate:', key, value.shape)
                    batched_encoding[key] = value
    ...
    def batch_encode_plus(
        ...
        **kwargs
        ...
        if answer_coordinates is None and answer_text is None:
    ...
    def _get_question_tokens(self, query):
        ...
        if self.max_question_length is not None and len(
                query_tokens) > self.max_question_length:
            logger.warning(
                "Skipping query as its tokens are longer than the max question length")
            return "", []
        if self.min_question_length is not None and len(
                query_tokens) < self.min_question_length:
            logger.warning(
        ...
        **kwargs
        ...
        for index, example in enumerate(
            ...
            outputs = self.prepare_for_model(
                ...
                verbose=verbose,
                prev_answer_coordinates=answer_coordinates[index -
                                                           1] if index != 0 else None,
                prev_answer_text=answer_text[index -
                                             1] if index != 0 else None,
            )
    ...
    def prepare_for_model(
        ...
        **kwargs
        ...
        if isinstance(padding, bool):
            if padding and (
                    max_length is not None or pad_to_multiple_of is not None):
        ...
        if truncation != TapasTruncationStrategy.DO_NOT_TRUNCATE:
            ...
            # print("after _get_truncated_table_rows", num_rows, num_tokens)
        ...
        table_ids = list(
            zip(*table_data))[0] if len(table_data) > 0 else list(zip(*table_data))
        ...
        column_ids = self.create_column_token_type_ids_from_sequences(
            query_ids, table_data)
        row_ids = self.create_row_token_type_ids_from_sequences(
            query_ids, table_data)
        ...
        # SECOND: add numeric-related features (and not parse them in these
        # functions):

        column_ranks, inv_column_ranks = self._get_numeric_column_ranks(
            column_ids, row_ids, raw_table, num_rows=num_rows)
        ...
        if answer_coordinates is not None and answer_text is not None:
            labels = self.get_answer_ids(
                ...
                answer_text,
                answer_coordinates)
            numeric_values = self._get_numeric_values(
        ...
        if max_length is None and len(
                encoded_inputs["input_ids"]) > self.model_max_length and verbose:
    ...
    def _get_token_budget(self, question_tokens, max_length=None):
        ...
        Args:
            question_tokens (`List[String]`):
                ...
                length.
        """
        return (max_length if max_length is not None else self.model_max_length) - \
            self._question_encoding_cost(question_tokens)

    def _get_table_values(self, table, num_columns, num_rows,
                          num_tokens) -> Generator[TableValue, None, None]:
        ...
        for tc in table.selected_tokens:
            ...
            while word_begin_index >= 0 and _is_inner_wordpiece(
                    cell[word_begin_index]):
                word_begin_index -= 1
    ...
    def _get_table_cost(self, table, num_columns, num_rows, num_tokens):
        return sum(
            1 for _ in self._get_table_values(
                table,
                num_columns,
                num_rows,
                num_tokens))
    ...
    def _get_max_num_tokens(
        ...
        # Need to manually swtich to the second block when running ITR
        # This is to disable cell truncation so that the length of returned
        # sequences can be correctly evaluated by ITR.

        if self.do_extreme_cell_truncation:
            num_tokens = 0
            ...
            if num_tokens < max_num_tokens:
                if self.cell_trim_length >= 0:
                    # We don't allow dynamic trimming if a cell_trim_length is
                    # set.
                    return None
                if num_tokens == 0:
                    return None
        else:
            _, _, current_num_tokens = self._get_table_boundaries(
                tokenized_table)
            cost = self._get_table_cost(
                tokenized_table,
                num_columns,
    ...
    def _get_num_columns(self, table):
        ...
        if num_columns >= self.max_column_id:
            num_columns = self.max_column_id - 1
        return num_columns
    ...
    def _get_cell_token_indexes(self, column_ids, row_ids, column_id, row_id):
        for index in range(len(column_ids)):
            if column_ids[index] - \
                    1 == column_id and row_ids[index] - 1 == row_id:
                yield index

    def _get_numeric_column_ranks(
            self,
            column_ids,
            row_ids,
            table,
        ...
        if table is not None:
            for col_index in range(len(table.columns)):
                ...
                try:
                    key_fn = get_numeric_sort_key_fn(
                        table_numeric_values.values())
                except ValueError:
                    continue

                table_numeric_values = {
                    row_index: key_fn(value) for row_index,
                    value in table_numeric_values.items()}
        ...
        if table is not None:
            ...
            for col_index in range(num_columns):
                for row_index in range(num_rows):
                    numeric_value = table.iloc[row_index,
                                               col_index].numeric_value
    ...
    def _get_all_answer_ids(self, column_ids, row_ids, answer_coordinates):
        ...
        return self._get_all_answer_ids_from_coordinates(
            column_ids, row_ids, answers_list=(
                _to_coordinates(answer_coordinates)))
    ...
    def get_answer_ids(
        ...
        if self.update_answer_coordinates:
            return self._find_answer_ids_from_answer_texts(
                column_ids, row_ids, tokenized_table, answer_texts=[
                    self.tokenize(at) for at in answer_texts_question], )
        return self._get_answer_ids(
            column_ids, row_ids, answer_coordinates_question)
    ...
    def _pad(
        ...
        needs_to_be_padded = (
            padding_strategy != PaddingStrategy.DO_NOT_PAD and len(
        ...
        if needs_to_be_padded:
            ...
            if self.padding_side == "right":
                ...
                if "numeric_values" in encoded_inputs:
                    encoded_inputs["numeric_values"] = encoded_inputs["numeric_values"] + [
                ...
                if "token_type_ids" in encoded_inputs:
                    encoded_inputs["token_type_ids"] = [
                        [self.pad_token_type_id] * 7] * difference + encoded_inputs["token_type_ids"]
                if "labels" in encoded_inputs:
                    encoded_inputs["labels"] = [0] * \
                        difference + encoded_inputs["labels"]
                if "numeric_values" in encoded_inputs:
                    encoded_inputs["numeric_values"] = [
                        float("nan")] * difference + encoded_inputs["numeric_values"]
                if "numeric_values_scale" in encoded_inputs:
                    encoded_inputs["numeric_values_scale"] = [
                        1.0] * difference + encoded_inputs["numeric_values_scale"]
                if "special_tokens_mask" in encoded_inputs:
                    encoded_inputs["special_tokens_mask"] = [
                        1] * difference + encoded_inputs["special_tokens_mask"]
                encoded_inputs["input_ids"] = [
                    self.pad_token_id] * difference + encoded_inputs["input_ids"]
            else:
    ...
    def _get_mean_cell_probs(
        ...
        return {coords: np.array(cell_probs).mean()
    ...
    def convert_logits_to_predictions(
        ...
        data = {key: value.numpy()
                for key, value in data.items() if key != "training"}
        ...
        # collect input_ids, segment ids, row ids and column ids of batch.
        # Shape (batch_size, seq_len)
        input_ids = data["input_ids"]
        segment_ids = data["token_type_ids"][:,
                                             :, token_types.index("segment_ids")]
        row_ids = data["token_type_ids"][:, :, token_types.index("row_ids")]
        column_ids = data["token_type_ids"][:,
                                            :, token_types.index("column_ids")]
...
# These are meant to be used in an academic setup, for production use
# cases Gold mine or Aqua should be used.


# taken from constants.py of the original implementation
# URL:
# https://github.com/google-research/tapas/blob/master/tapas/utils/constants.py
...
def parse_text(text):
    ...
    spans = sorted(
        span_dict.items(),
        key=lambda span_value: _get_span_length_key(
            span_value[0]),
        reverse=True)
    selected_spans = []
...
_PrimitiveNumericValue = Union[float,
...
def _get_value_as_primitive_value(numeric_value):
    ...
    if numeric_value.date is not None:
        ...
        # All dates fields are cased to float to produce a simple primitive
        # value.
