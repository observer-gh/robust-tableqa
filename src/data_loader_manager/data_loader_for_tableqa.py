...
class DataLoaderForTableQA(DataLoaderWrapper):
        ...
        for split, split_file_path in module_config.config.path.items():
            ...
            if os.path.exists(split_file_path):  # cache
                ...
                if 'use_original_tapas_data' in module_config.config.preprocess:
                    ...
                    tf_input_path = os.path.join(
                        self.config.DATA_FOLDER, tf_input_path)
                    ...
                    for example in tqdm(tf_dataset['examples']):
                        question_id = example['question_id'].replace(
                            '-0_0', '')
                    ...
                    dataset = dataset.map(
                        attach_tf_input_to_dataset,
                        batched=False,
                        desc="attach_tf_input_to_dataset")

                    conversion_success = len(
                        dataset.filter(lambda x: x["valid"]))
                    conversion_error = len(
                        dataset.filter(
                            lambda x: x["valid"] == False))

                    # if is_training:
                    #     dataset = dataset.filter(lambda x: x["valid"] == True)

                    logger.info(
                        f'conversion_success {conversion_success} / conversion_error {conversion_error} --> {conversion_success/(conversion_success+conversion_error)*100}%')
                ...
                if "transform_to_sqa_format" in module_config.config.preprocess:
                    ...
                    def process_to_sqa_format(example):
                        ...
                        table = example['table']
                        # Transform header/row format into a format that
                        # parse_tapas_question can take

                        new_table = prepare_table_in_sqa_format(table)

                        compact_table_pd = pd.DataFrame.from_dict(
                            new_table, orient='index', columns=table['header'])

                        try:
                            question, answer_texts, answer_coordinates, float_value, aggregation_function = parse_tapas_question(
                                table=compact_table_pd, question=example['question'], answer_texts=example['answers'])
                            ...
                            # the answer_coordinates must be a single list of
                            # one or more tuples. Each tuple must be a
                            # (row_index, column_index) pair.
                            example['answer_coordinates'] = [
                                (coord[1], coord[0]) for coord in answer_coordinates]
                    ...
                    dataset = dataset.map(
                        process_to_sqa_format,
                        batched=False,
                        num_proc=4,
                        desc="transform_to_sqa_format")
                    conversion_success = len(
                        dataset.filter(lambda x: x["valid"]))
                    conversion_error = len(
                        dataset.filter(
                ...
                if "check_tapas_tokenization_compatibility" in module_config.config.preprocess:
                    def test_encoding(example):
                        if not example['valid']:
                            ...
                            # since they have been dropped in the WTQ-to-SQA
                            # conversion
                            return example
                        ...
                        try:
                            if is_training:
                                ...
                                # i.e. in supervision mode, if answer_text is
                                # provided, answer_coordinates can not be None
                                ...
                                # for testing, we allow all data to appear, as
                                # long as there is no encoding error
                                encoding = self.tokenizer(
                                    table=compact_table_pd,
                                    queries=example['question'],
                                    truncation=True,
                    ...
                    if is_training:
                        dataset = dataset.map(
                            ...
                            batched=False,
                            num_proc=8,
                            desc='check_tapas_tokenization_compatibility')
                    else:
                        logger.info(
                            'skip compatibility check for val/test sets...')
                    ...
                    def split_table_by_column_combination(example):
                        ...
                        for max_column in range(1, MAX_COLUMNS + 1):
                            for comb in combinations(
                                    list(range(num_columns)), max_column):
                                selected_columns = [
                                    table['header'][i] for i in comb]
                                selected_rows = [[row[i] for i in comb]
                                                 for row in table['rows']]
                                selected_types = [
                                    'unknown'] * len(selected_columns)
                                ...
                                if set(comb).issubset(set(gold_col)):
                                    # if the column of this subtable is a gold
                                    # col
                        ...
                        example['num_positive_sub_tables'] = len(
                            positive_subtables)
                        example['num_negative_sub_tables'] = len(
                            negative_subtables)
                        example['num_sub_tables'] = len(
                            positive_subtables) + len(negative_subtables)
                        return example

                    dataset = dataset.map(
                        split_table_by_column_combination,
                        batched=False,
                        num_proc=4,
                        desc="split_table_by_column_combination")
                ...
                if 'split_table_by_row_combination' in module_config.config.preprocess:
                    def split_table_by_row_combination(example):
                        # this function splits "table" into "row-wise
                        # sub-tables"
                        table = example["table"]
                        positive_subtables = []
                        negative_subtables = []
                        ...
                        for max_row in range(1, MAX_ROWS + 1):
                            for comb in combinations(
                                    list(range(num_rows)), max_row):
                                selected_columns = table['header']
                                selected_rows = [table['rows'][i]
                                                 for i in comb]
                                selected_types = [
                                    'unknown'] * len(selected_columns)
                                sub_table = deepcopy(table)
                    ...
                    dataset = dataset.map(
                        ...
                        batched=False,
                        num_proc=4,
                        desc="split_table_by_row_combination")
                ...
                if 'split_table_by_mixed_combination' in module_config.config.preprocess:
                    def split_table_by_mixed_combination(example):
                        # this function splits "table" into "column-wise and
                        # row-wise sub-tables"
                        table = example["table"]
                        positive_subtables = []
                        negative_subtables = []
                    ...
                    dataset = dataset.map(
                        ...
                        batched=False,
                        num_proc=4,
                        desc="split_table_by_mixed_combination")
            ...
            if 'valid' not in dataset.column_names:
                ...
                conversion_success = len(dataset.filter(lambda x: x["valid"]))
                conversion_error = len(
                    dataset.filter(
                        lambda x: x["valid"] == False))
        ...
        for split, split_file_path in module_config.config.path.items():
            ...
            if os.path.exists(split_file_path):
                ...
                def preprocess_tableqa_function(example):
                    ...
                    answer_list, answer_coordinates, aggregation_op, conditions = retrieve_wikisql_query_answer_tapas(
                        tapas_table, example_sql)

                    example['conditioned_columns'] = [
                        condition.column for condition in conditions]
                    example['num_conditioned_columns'] = len(
                        example['conditioned_columns'])
                ...
                dataset = dataset.map(
                    ...
                    batched=False,
                    num_proc=4,
                    desc="preprocess_tableqa_function")
                ...
                print(
                    dataset.to_pandas().value_counts(
                        subset=['aggregation_function']))
                ...
                if 'use_original_tapas_data' in module_config.config.preprocess:
                    ...
                    all_tf_data = {}
                    for example in tqdm(tf_dataset['examples']):
                        table_id = example["table_id"].replace(
                    ...
                    def attach_tf_input_to_dataset(example):
                        ...
                        question = example['question'].lower()
                        tf_input_example = all_tf_data.get(
                            table_id + ":" + question, None)
                        if tf_input_example is not None:
                ...
                if 'move_answers_to_table_end' in module_config.config.preprocess:
                    def move_answers_to_table_end(example):
                        ...
                        gold_col = [
                            coord[1] for coord in answer_coordinates] + [example['sql']['sel']]
                        gold_col = list(set(gold_col))
                        conditioned_col = list(
                            set(example['conditioned_columns']))
                        gold_row = list(set([coord[0]
                                        for coord in answer_coordinates]))

                        original_row_ids = list(
                            range(len(example_table["rows"])))
                        original_column_ids = list(
                            range(len(example_table["header"])))
                        ...
                        column_index_to_content = {
                            ...
                            header_name in enumerate(
                                example_table['header'])}

                        # Change the table
                        new_table = table.iloc[shifted_row_ids,
                                               shifted_column_ids]
                        new_example_table = deepcopy(example_table)
                        new_example_table['header'] = [
                            column_index_to_content[index] for index in shifted_column_ids]
                        new_example_table['rows'] = [
                            row.tolist()[1:] for row in new_table.to_records()]
                        new_example_table['types'] = [
                            example_table['types'][index] for index in shifted_column_ids]

                        # Create mapping from original column index to current
                        # column index
                        column_mapping = {
                            f: t for f, t in zip(
                                shifted_column_ids, original_column_ids)}
                        row_mapping = {
                            f: t for f, t in zip(
                                shifted_row_ids, original_row_ids)}
                        ...
                        example['conditioned_columns'] = [column_mapping[i]
                                                          for i in example['conditioned_columns']]
                    ...
                    dataset = dataset.map(
                        ...
                        batched=False,
                        num_proc=4,
                        desc="move_answers_to_table_end")
                ...
                if 'move_answers_to_table_end_column_wise' in module_config.config.preprocess:
                    def move_answers_to_table_end_column_wise(example):
                        ...
                        return example
                    dataset = dataset.map(
                        move_answers_to_table_end_column_wise,
                        batched=False,
                        num_proc=4,
                        desc="move_answers_to_table_end_column_wise")
                    ...
                    def split_table_by_column(example):
                        ...
                        for i in range(num_columns):
                            for j in range(i + 1, num_columns):
                                selected_columns = [
                                    table['header'][i], table['header'][j]]
                                selected_rows = [[row[i], row[j]]
                                                 for row in table['rows']]
                                selected_types = [
                                    table['types'][i], table['types'][j]]
                                sub_table = deepcopy(table)
                                sub_table['header'] = selected_columns
                                sub_table['rows'] = selected_rows
                                sub_table['types'] = selected_types
                ...
                if 'remove_no_pos_samples_for_training' in module_config.config.preprocess:
                    dataset = dataset.filter(
                        lambda x: x["num_positive_sub_tables"] > 0)
    ...
    def LoadNQTableData(self, module_config):
        ...
        for split, split_path in module_config.config.table_path.items():
            ...
            split_cache_path = os.path.join(
                self.config.DATA_FOLDER, split_cache_path)
            if os.path.exists(split_cache_path):
                ...
                with open(split_path, "r") as json_file:
                    for line in tqdm(json_file):
                        ...
                        rows = [[i['text'] for i in row['cells']]
                ...
                table_dataset = pd.DataFrame.from_dict(
                    all_tables, orient='index')
                table_dataset = Dataset.from_pandas(table_dataset)
                table_dataset = table_dataset.remove_columns(
                    ['__index_level_0__'])
        ...
        for split, split_path in module_config.config.data_path.items():
            ...
            if os.path.exists(split_cache_path):
                ...
                if self.data.nq_tables_data.get('bm25_results', None) is None:
                    bm25_results_path = os.path.join(
                        self.config.DATA_FOLDER, module_config.config.bm25_results)
                ...
                with open(split_path, "r") as json_file:
                    for line in tqdm(json_file):
                        ...
                        alternative_table_ids = line_data['table'].get(
                            'alternativeTableIds', [])
                        ...
                        for question_dict in line_data['questions']:
                            ...
                            alternative_answers = []
                            for alternative_answer in question_dict.get(
                                    'alternativeAnswers', []):
                            ...
                            bm25_retrieved_items = bm25_results.get(
                                question_id, None)
                            if bm25_retrieved_items:
                                bm25_retrieved_item_ids = [
                                    i['id'] for i in bm25_retrieved_items]
                            else:
                                logger.error(
    ...
    def LoadE2EWTQData(self, module_config):
        ...
        if self.data.e2e_wtq_data.get('bm25_results', None) is None:
            bm25_results_path = os.path.join(
                self.config.DATA_FOLDER,
                module_config.config.bm25_results)
            if not os.path.exists(bm25_results_path):
                # raise Exception("Please generate BM25 results first.")
                bm25_results = {}
            else:
