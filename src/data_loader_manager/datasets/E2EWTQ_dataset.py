...
class E2EWTQDataset(WikiTQDataset):
    ...
    def __init__(self, config, dataset_dict):
        ...
        self.tables = self.data['table_dataset'].to_pandas(
        ).set_index("table_id").to_dict(orient="index")

    def collate_fn(self, batch):
        '''
        when collate_fn is given to the torch dataloader,
        we can do further actions to the batch, e.g., tensor can be formed here
        a batch is formed as a list where each element is a
        defined data returned by __getitem__, andy
        '''
...
class DPRE2EWTQDataset(E2EWTQDataset):
    ...
    def __getitem__(self, idx):
        ...
        # With some prob the negative samples are harvested from the
        ...
        if random.uniform(0, 1) < bm25_ratio:
            valid_ids = [
                i for i in sample.bm25_retrieved_item_ids if i != sample.pos_item_id]
            if bm25_top_k > 0:
