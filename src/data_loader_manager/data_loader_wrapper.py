...
class DataLoaderWrapper():
    ...
    def __init__(self, config):
        ...
        if self.config.model_config.base_model == 'ColBERT':
            ...
            self.SPECIAL_TOKENS['additional_special_tokens'] = self.tokenizer.tok.additional_special_tokens + \
            ...
            self.DECODER_SPECIAL_TOKENS['additional_special_tokens'] = self.decoder_tokenizer.tok.additional_special_tokens + \
                self.DECODER_SPECIAL_TOKENS['additional_special_tokens']
            self.decoder_tokenizer.tok.add_special_tokens(
                self.DECODER_SPECIAL_TOKENS)

        elif self.config.model_config.get("QueryEncoderModelClass", "") == 'ColBERT':
            # In RAG with ColBERT, only init query tokenizer with ColBERT
            # tokenizer
            logger.info("initializing ColBERT tokenizers...")
            ...
            self.DECODER_SPECIAL_TOKENS = self.config.model_config.DECODER_SPECIAL_TOKENS
            self.DECODER_SPECIAL_TOKENS['additional_special_tokens'] = self.decoder_tokenizer.additional_special_tokens + \
                self.DECODER_SPECIAL_TOKENS['additional_special_tokens']
            ...
            self.SPECIAL_TOKENS = self.config.model_config.SPECIAL_TOKENS
            self.SPECIAL_TOKENS['additional_special_tokens'] = self.tokenizer.additional_special_tokens + \
                self.SPECIAL_TOKENS['additional_special_tokens']
            ...
            if self.config.model_config.get(
                    'DecoderTokenizerClass', None) is not None:
                DecoderTokenizerClass = globals(
                )[self.config.model_config.DecoderTokenizerClass]
                self.decoder_tokenizer = DecoderTokenizerClass.from_pretrained(
                    self.config.model_config.DecoderTokenizerModelVersion)
