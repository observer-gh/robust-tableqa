...
class ModuleParser():
    ...
    obj['attributes'], obj['attribute_scores']):
            ...
            return_dict.text_sequence = ' '.join([module.separation_tokens.start] + [
                                                 sample.img_caption['caption']] + [module.separation_tokens.end])
    ...
    sample: EasyDict,
     module: EasyDict) -> Optional[EasyDict]:
        ...
        return_dict.text_sequence = ' '.join(
            [module.separation_tokens.start] + [sample.passage_content] + [module.separation_tokens.end])
        return return_dict
    ...
    sample: EasyDict,
     module: EasyDict) -> Optional[EasyDict]:
        ...
        for column_name in table['header']:
            table_sentences += [column_name,
                                module.separation_tokens.header_sep]
...
if isinstance(value,                     if )                        # automatically concatenate strings with the same key
    ...
    self, data_to_process: EasyDict) -> EasyDict:
        ...
        # The tokenizer supports one sample at a time, so we have to process
        # one by one
        for i in range(len(tables)):
            ...
            encoding = EasyDict({key: val.squeeze(0)
